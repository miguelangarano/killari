...
class Email{
    ...
    public function sendEmail($proxy, $obj, $kind){
        ...
        try {
            ...
            if($kind==1){
                $mail->addAddress($obj->email, 'Cliente');     // Add a recipient
            }else{
    ...
    public function sendErrorEmail($proxy, $obj){
        $mail = new PHPMailer(true);                              // Passing `true` enables exceptions
        try {
            //Server settings
            $mail->SMTPDebug = 2;                                 // Enable verbose debug output
            ...
            $mail->Subject = 'Reservacion de Killari Spa';
            //echo ('proxy: '.$proxy->getProxy());
            $homepage = file_get_contents($proxy.'libs/phpmailer/formatoerror.html');
            $variables=array();
            $variables['nombre']=$obj->nombre;
            $variables['apellido']=$obj->apellido;
            $variables['fecha']=$obj->fecha;
